...
from bluzelle.codec.cosmos.auth.v1beta1.query_pb2 import (QueryAccountRequest,
                                                          QueryAccountResponse)
from google.protobuf import json_format
from bluzelle.codec.cosmos.auth.v1beta1.query_pb2_grpc import QueryStub
from bluzelle.codec.cosmos.tx.signing.v1beta1.signing_pb2 import (
    SIGN_MODE_DIRECT, SignMode)
from bluzelle.codec.cosmos.tx.v1beta1.tx_pb2 import Fee, Tx
...
class TxCallable(Callable):
    """TxCallable will be used to recieving grpc calls from the user and sending them back to the 
    ...
    def __init__(
        ...
        super().__init__(
            tendermint34Client, method, request_serializer, response_deserializer
        )
...
class TransactionClient(RpcChannel):
    ...
    def with_transactions(self, messages: List[Message]) -> bytes:
        """Sending multiple grpc Messages at once, and wait for it to be included in a block"""
        signed_tx = self.prepair_transaction(messages)
        
        # Sending transaction on-chain and recieving the tx hash.
        tx_hash = self.submit_transaction(signed_tx)
        
        # Block untill the transaction is included in a block succesfuly or failed.
        self.wait_transaction_done(tx_hash)

    def prepair_transaction(self, messages: List[Message]):
        """Creating a offline signed transaction using input messages and wallet data"""
        ...
        fee = Fee(gas_limit=self.max_gas, amount=[Coin(denom="ubnt", amount=str(int(self.max_gas * self.gas_price)))])
        ...
        while time.time() < timeout:
            ...
            if int(response['total_count']) > 0:
                ...
                if tx['code'] != 0 or len(tx['events']) == 0:
                    ...
                    raise ValueError(f"call failed with code {code} (log: {log}, codespace: {code_space}))")
            # Waiting 20 seconds before making another rpc call.
            time.sleep(20)

    def get_account(self) -> BaseAccount:
        """Getting account information by making a Account rpc call using the tendermint client.
        ...
        if account.address != self.wallet.address:
            raise ValueError("Resulting account from abci_query should be equal to the wallet address!")
        return account
